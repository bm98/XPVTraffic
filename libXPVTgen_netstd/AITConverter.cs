...
{
  /// <summary>
  /// Converts an AIT stream to a route script
  /// </summary>
  public class AITConverter
  {
    ...
    private CmdList RouteFromAIT( List<string> aitStream, bool absoluteTrack, bool ignoreAirborne )
    {
      ...
      if ( vac == null ) { Error = "AIT message conversion error"; return null; }; // converter error
    ...
    public bool CreateRouteScript( string aitFile, bool absoluteTrack, bool ignoreAirborne )
    {
      ...
      var route = RouteFromAIT( aitStream, absoluteTrack, ignoreAirborne );
      if ( route == null ) return false;

      var ret = true;
      using ( var sw = new StreamWriter( aitFile + ".vsc", false ) ) {
        sw.WriteLine( $"# XPVTraffic-AITConverter: {DateTime.Now.ToString( "s" )}" );
